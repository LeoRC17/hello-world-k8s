pipeline {
  ...
  stages {
    ...
    stage('Deploy') {
      ...
      steps {
        script {
          // ensure artifact exists (use shell env variables)
          sh '''
          ...
          withCredentials([usernamePassword(credentialsId: env.NEXUS_CREDENTIALS_ID,
            ...
            // use single-quoted sh block so Groovy does not interpolate secrets; unquoted heredoc lets shell expand $NEXUS_USER/$NEXUS_PASS
            sh '''
...
EOF
              ...
              # show that settings file exists (credentials will be masked in console)
